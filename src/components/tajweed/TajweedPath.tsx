...
export function TajweedPath(): JSX.Element {
  ...
  return (
    <div className="space-y-4">
      ...
      <ol className="relative space-y-3 before:absolute before:left-[19px] before:top-4 before:bottom-4 before:w-px before:bg-[var(--border)]">
        {TAJWEED_CONCEPTS.map((concept) => {
          ...
          return (
            <li key={concept.id} className="relative pl-12">
              <span
                aria-hidden="true"
                className={[
                  'absolute left-0 top-2 inline-flex h-10 w-10 items-center justify-center rounded-full border-2 text-sm font-extrabold tnum transition-all duration-200',
                  isMastered
